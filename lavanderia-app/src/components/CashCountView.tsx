...
export function CashCountView() {
  ...
  const [totals, setTotals] = useState({
    income: 0,
    expense: 0,
    expected: 0 // Utilidad Histórica Acumulada
  });
  ...
  // 1. Obtener la Utilidad HISTÓRICA (Acumulado desde el inicio hasta la fecha seleccionada)
  const fetchSystemProfit = async () => {
    const { data, error } = await supabase
      .from('transactions')
      .select('amount, type')
      .lte('date', selectedDate); // <--- CAMBIO CLAVE: "lte" significa Menor o Igual (Todo el historial)
    ...
    if (data) {
      // Sumamos ingresos históricos
      const totalIncome = data
        .filter((t: any) => t.type === 'income')
        .reduce((sum: number, t: any) => sum + Number(t.amount), 0);
        
      // Sumamos gastos históricos
      ...
      setTotals({
          ...
          expected: totalIncome - totalExpense // Utilidad Histórica
      });
    }
  };

  // 2. Calcular suma de billetes
             ...
             {['1000', '500', '200', '100', '50', '20', '10', '5', '2', '1', '0.5'].map(val => (
               <div key={val} className="flex items-center gap-4">
                  ...
                  <input 
                    ...
                    min="0"
                    placeholder="0"
                    className="flex-1 border border-slate-200 rounded-lg p-2 text-center font-mono font-bold text-blue-900 bg-slate-50 focus:bg-white transition"
